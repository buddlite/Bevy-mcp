...
# -----------------------------------------------------------------------------
# S2 + S3 protocol: access provenance and scoped tracking
# -----------------------------------------------------------------------------
path = 'crates/bevy-mcp-core/src/advanced.rs'
t = read(path)
t = replace_once(t, '''    SystemInspect {
        system: String,
...
''', '''    SystemInspect {
    ...
    TrackingStatus,
    SystemTimings {
''', 'advanced access/tracking variants')
write(path, t)

# New scoped tracker implementation.
write('crates/bevy-mcp-host/src/change_tracking.rs', r'''use std::collections::{HashMap, HashSet, VecDeque};
...
impl WorldChangeTracker {
    ...
    fn should_track_component(&self, name: &str) -> bool {
        if self.exclude_components.iter().any(|v| component_name_matches(name, v)) {
            return false;
        }
        self.mode == TrackingMode::Full
            || self.components.iter().any(|v| component_name_matches(name, v))
            || self.dynamic_components.iter().any(|v| component_name_matches(name, v))
    }

    fn should_track_resource(&self, name: &str) -> bool {
        if self.exclude_resources.iter().any(|v| component_name_matches(name, v)) {
            return false;
        }
        self.mode == TrackingMode::Full
            || self.resources.iter().any(|v| component_name_matches(name, v))
            || self.dynamic_resources.iter().any(|v| component_name_matches(name, v))
    }

    pub fn changes_since(&self, frame: u64) -> Value {
        let frames: Vec<Value> = self.history.iter().filter(|e| e.frame > frame).map(frame_changes_json).collect();
        json!({
            "since_frame": frame,
    ...
    pub fn component_changed_last_frame(&self, entity: Entity, component: &str) -> bool {
        self.latest_changed_components.iter().any(|(e, c)| *e == entity && component_name_matches(c, component))
    }
...
pub fn track_world_changes(world: &mut World) {
    ...
    let mut changes = FrameChanges { frame, ..Default::default() };

    for entity_ref in world.iter_entities() {
        if entity_ref.contains::<IsResource>() { continue; }
        let entity = entity_ref.id();
        let mut snapshot = EntitySnapshot::default();
        for component_id in entity_ref.archetype().components() {
            let name = component_name(world, *component_id);
            if !tracker.should_track_component(&name) { continue; }
            let Some(ticks) = entity_ref.get_change_ticks_by_id(*component_id) else { continue; };
            snapshot.components.insert(*component_id, TickSnapshot { added: ticks.added.get(), changed: ticks.changed.get() });
        }
                ...
                Some(previous) => {
                    for (component_id, ticks) in &snapshot.components {
                        ...
                        match previous.components.get(component_id) {
                            None => changes.components.push(ComponentChangeRecord { entity, component: name, kind: ChangeKind::Added }),
                            Some(old) if old.changed != ticks.changed => changes.components.push(ComponentChangeRecord { entity, component: name, kind: ChangeKind::Changed }),
                            _ => {}
                        }
                    }
                    for component_id in previous.components.keys() {
                        if !snapshot.components.contains_key(component_id) {
                            changes.components.push(ComponentChangeRecord { entity, component: component_name(world, *component_id), kind: ChangeKind::Removed });
                        }
                    }
                }
            }
    ...
    if tracker.initialized {
        for entity in tracker.previous_entities.keys() {
            if !current_entities.contains_key(entity) { changes.despawned.push(*entity); }
        }
    }
    ...
    for (info, _) in world.iter_resources() {
        ...
        if is_internal_mcp_resource(&name) || !tracker.should_track_resource(&name) { continue; }
        let component_id = info.id();
        let Some(ticks) = world.get_resource_change_ticks_by_id(component_id) else { continue; };
        let snapshot = TickSnapshot { added: ticks.added.get(), changed: ticks.changed.get() };
        current_resource_names.insert(component_id, name.clone());
        if tracker.initialized {
            match tracker.previous_resources.get(&component_id) {
                None => changes.resources.push(ResourceChangeRecord { resource: name, kind: ChangeKind::Added }),
                Some(old) if old.changed != snapshot.changed => changes.resources.push(ResourceChangeRecord { resource: name, kind: ChangeKind::Changed }),
                _ => {}
            }
        }
    ...
    if tracker.initialized {
        for component_id in tracker.previous_resources.keys() {
            if !current_resources.contains_key(component_id) {
                let name = tracker.resource_names.get(component_id).cloned().unwrap_or_else(|| format!("resource#{}", component_id.index()));
                changes.resources.push(ResourceChangeRecord { resource: name, kind: ChangeKind::Removed });
            }
        }
    }

    tracker.latest_changed_components = changes.components.iter()
        .filter(|c| matches!(c.kind, ChangeKind::Added | ChangeKind::Changed))
        .map(|c| (c.entity, c.component.clone())).collect();
    ...
    if tracker.initialized {
        ...
        while tracker.history.len() > tracker.max_history { tracker.history.pop_front(); }
    } else {
...
fn component_name(world: &World, component_id: ComponentId) -> String {
    world.components().get_info(component_id).map(|i| i.name().to_string()).unwrap_or_else(|| format!("component#{}", component_id.index()))
}
